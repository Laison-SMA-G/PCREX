...
export default function Home() {
  ...
  async function handleRestock(productId) {
    ...
    if (amount <= 0) return toast.error("Enter a valid restock amount");
          ...
          {lowStock.length === 0 ? (
            ...
            <ul className="space-y-2">
              {lowStock.map((p) => (
                <li
                  key={p._id}
                  className="flex flex-col md:flex-row md:items-center md:justify-between border p-2 rounded bg-white"
                >
                  <div>
